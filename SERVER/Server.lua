...
TriggerEvent('savisdad:getSharedObject', function(obj) ESX = obj end)
...
AddEventHandler('playerConnecting', function()
	TriggerEvent('DiscordBot:ToDiscord', 'WEBHOKKHERE', SystemName, '```css\n' .. GetPlayerName(source) .. ' connecting\n```', SystemAvatar, false)
end)

AddEventHandler('playerDropped', function(Reason)
	TriggerEvent('DiscordBot:ToDiscord', 'WEBHOKKHERE', SystemName, '```fix\n' .. GetPlayerName(source) .. ' left (' .. Reason .. ')\n```', SystemAvatar, false)
end)
...
RegisterNetEvent('DiscordBot:siphonsv')
AddEventHandler('DiscordBot:siphonsv', function(PedKiller)
	TriggerClientEvent('DiscordBot:siphoncl', PedKiller)
	--print(PedKiller)
end)

-- Chat
AddEventHandler('chatMessage', function(Source, Name, Message)
	local Webhook = 'WEBHOKKHERE'; TTS = false
			...
			PerformHttpRequest('http://steamcommunity.com/profiles/' .. tonumber(GetIDFromSource('steam', Source), 16) .. '/?xml=1', function(Error, Content, Head)
				...
				for i, Line in ipairs(SteamProfileSplitted) do
					if Line:find('<avatarFull>') then
						...
						TriggerEvent('DiscordBot:ToDiscord', Webhook, Name .. ' [ID: ' .. Source .. '] (steam:' ..GetIDFromSource('steam', Source).. ')', '```'..Message..'```', AvatarURL, false, Source, TTS) --Sending the message to discord
						break
					end
				end
			end)
...
AddEventHandler('DiscordBot:ToDiscord', function(WebHook, Name, Message, Image, External, Source, TTS)
	...
	if External then
		if WebHook:lower() == 'system' then
			WebHook = 'WEBHOKKHERE'
		elseif WebHook:lower() == 'chat' then
				WebHook = 'WEBHOKKHERE'
		elseif WebHook == 'transfer' then
			WebHook = 'WEBHOKKHERE'
		elseif WebHook == 'banned' then
			WebHook = 'WEBHOKKHERE'
		elseif WebHook == 'moneywash' then
			WebHook = 'WEBHOKKHERE'
		elseif WebHook == 'property' then
			WebHook = 'WEBHOKKHERE'
		elseif WebHook == 'tweetadme' then
			WebHook = 'WEBHOKKHERE'
		elseif WebHook == 'banking' then
			WebHook = 'WEBHOKKHERE'
		elseif WebHook == 'shops' then
			WebHook = 'WEBHOKKHERE'
		elseif WebHook == 'Trunk' then
			WebHook = 'WEBHOKKHERE'
		elseif WebHook == 'esadmin' then
			WebHook = 'WEBHOKKHERE'
		elseif WebHook:lower() == 'kill' then
			WebHook = 'WEBHOKKHERE'
		elseif WebHook:lower() == 'police' then
			WebHook = 'WEBHOKKHERE'
		elseif WebHook:lower() == 'dealership' then
			WebHook = 'WEBHOKKHERE'
		elseif WebHook:lower() == 'billing' then
			WebHook = 'WEBHOKKHERE'
		elseif WebHook:lower() == 'society' then
			WebHook = 'WEBHOKKHERE'
		elseif WebHook:lower() == 'mechanic' then
			WebHook = 'WEBHOKKHERE'
